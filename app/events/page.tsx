...
export default function EventsPage() {
  ...
  const allMonths: { id: MonthId; name: string }[] = [
  ...
  const currentMonthId: MonthId = "july";
  const currentMonthIndex = allMonths.findIndex(
    (month) => month.id === currentMonthId
  );
  const months: { id: MonthId; name: string }[] = allMonths.slice(currentMonthIndex);

  const [selectedMonth, setSelectedMonth] = useState<MonthId>("july");
  ...
  const getVisibleMonths = (): { id: MonthId; name: string }[] => {
  ...
  const eventsData: Record<MonthId, EventItem[]> = {
    january: [],
    february: [],
    march: [],
    april: [],
    ...
    <div className="min-h-screen bg-[#050a18]">
        ...
        <div className="container mx-auto text-center">
          ...
          >
            ...
            <p className="text-xl md:text-2xl text-gray-300 max-w-3xl mx-auto mb-8">
              Discover exciting events, tournaments, and activities happening at
              Destination KP throughout the year.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="pb-20 px-4 md:px-6">
        <div className="container mx-auto">
          <Tabs
            value={selectedMonth}
            onValueChange={(val: string) => setSelectedMonth(val as MonthId)}
            className="w-full"
          >
            {/* Month Navigation */}
            <div className="mb-12">
              <div className="flex items-center justify-center gap-2 sm:gap-4 mb-4">
                ...
                <div className="flex-1 max-w-xs sm:max-w-md md:max-w-lg lg:max-w-2xl">
                  <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 md:grid-cols-4 bg-gray-800/50 backdrop-blur-sm border border-gray-700 gap-1">
                    {getVisibleMonths().map((month: { id: MonthId; name: string }) => (
                      <TabsTrigger
                        key={month.id}
                        value={month.id}
              ...
              <div className="text-center text-xs sm:text-sm text-gray-400 px-4">
                <span className="hidden sm:inline">
                  ...
                  -{" "}
                  {Math.min(
                    (currentMonthIndexForView + 1) * monthsPerView,
                    months.length
                  )}{" "}
                  of {months.length}
                </span>
              ...
              >
                ...
                >
                  <h2 className="text-3xl font-bold text-white mb-8 text-center">
                    Events in {month.name}
                  </h2>

                  {eventsData[month.id] && eventsData[month.id].length > 0 ? (
                    <div className="grid gap-8 md:gap-12">
                      {eventsData[month.id].map((event: EventItem, index: number) => (
                        <React.Fragment key={event.id}>
                          ...
                          {index % 2 === 0 &&
                            index !== (eventsData[month.id] as EventItem[]).length - 1 && (
                              <SliderNobutton
                                slides={promoSlides}
                                {...sliderConfig}
                                fit="contain"
                    ...
                    <div className="text-center py-16">
                      ...
                      <p className="text-gray-500">
                        Check back later for upcoming events in {month.name}.
                      </p>
                    </div>
                  )}
                </motion.div>
